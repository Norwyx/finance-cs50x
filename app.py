...
def index():
    ...
    totals = [transaction["total"] for transaction in transactions_db]
    grand_total = sum(totals)

    return render_template("index.html", database=transactions_db, cash=cash, grand_total=grand_total)
...
def buy():
    ...
    if request.method == "GET":
        ...
        if not isinstance(shares, int) or shares < 1:
            return apology("not valid input", 403)
        ...
        if user_cash < total_cost:
            ...
            uptd_cash = user_cash - total_cost
            db.execute("UPDATE users SET cash = ? WHERE id = ?", uptd_cash, user_id)

            db.execute("INSERT INTO transactions (user_id, symbol, shares, price, transacted) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)", user_id, stock["symbol"], shares, stock["price"])
...
def sell():
    ...
    if request.method == ("GET"):
        ...
        if shares > user_shares:
            return apology("Invalid amount of stocks", 403)

        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, transacted) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)", user_id, stock["symbol"], (-1)*shares, stock["price"])
